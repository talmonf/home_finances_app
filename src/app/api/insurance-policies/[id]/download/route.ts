...
export async function GET(
  ...
  try {
    ...
    const row = await prisma.insurance_policies.findFirst({
      ...
      select: {
        policy_file_name: true,
        policy_file_mime_type: true,
    ...
    if (!row.policy_storage_bucket || !row.policy_storage_key) {
      return NextResponse.json({ error: "No policy file uploaded." }, { status: 404 });
    }
    ...
    const result = await client.send(
      new GetObjectCommand({
        Bucket: row.policy_storage_bucket,
        Key: row.policy_storage_key,
      }),
    );
    ...
    const fileName = row.policy_file_name || "policy-document";
    ...
    headers.set("Content-Type", row.policy_file_mime_type || "application/octet-stream");
    ...
    console.error("Insurance policy file download failed:", error);
