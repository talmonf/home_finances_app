...
export function FamilyMembersFormSection({ labels, linkableClients, initialRows, initialMainSlotIndex }: Props) {
  ...
  const [draftPosition, setDraftPosition] = useState<FamilyMemberPosition>("father");
  ...
  const saveModal = useCallback(() => {
    ...
    setRows((prev) => [...prev, { key: newKey(), kind: "new", firstName: name, position: draftPosition }]);
    if (wasEmpty) setMainSlotIndex(0);
    setModalOpen(false);
  }, [draftFirst, draftPosition, rows.length]);
    ...
    <div className="space-y-3 md:col-span-2">
      ...
      {rows.length === 0 ? (
        ...
        <ul className="divide-y divide-slate-800 rounded-lg border border-slate-700 bg-slate-950/40">
          {rows.map((r, i) => (
            <li key={r.key} className="flex flex-wrap items-center gap-3 px-3 py-2 text-sm">
      ...
      <details className="rounded-lg border border-slate-800 bg-slate-950/30 p-2 text-slate-500">
        ...
        <p className="mt-2 text-xs text-slate-500">{labels.advancedHint}</p>
        <label className="mt-2 block text-[11px] uppercase tracking-wide text-slate-600">{labels.linkExistingLabel}</label>
        <select
          multiple
          ...
          >
            ...
            <div className="mt-3 space-y-3">
              ...
              <div className="space-y-1">
                ...
                <select
                  value={draftPosition}
                  onChange={(e) => setDraftPosition(e.target.value as FamilyMemberPosition)}
                  className="w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-sm text-slate-100"
                >
