...
function parseOptionalKm(raw: string | null | undefined): string | null {
  const value = raw?.trim();
  if (!value) return null;
  const parsed = Number(value.replace(",", "."));
  if (!Number.isFinite(parsed) || parsed < 0) return null;
  ...
  await prisma.$transaction(async (tx) => {
    await tx.therapy_clients.create({
      data: {
        ...
        end_reason,
        notes: (formData.get("notes") as string)?.trim() || null,
        team_members: (formData.get("team_members") as string)?.trim() || null,
        default_job_id,
        default_program_id,
        default_visit_type,
        default_session_length_minutes,
...
export async function createTherapyTreatment(formData: FormData) {
  ...
  let appointmentToComplete: {
    id: string;
    family_id: string | null;
    client_id: string;
    job_id: string;
    program_id: string | null;
    visit_type: NonNullable<typeof visit_type>;
  ...
  if (appointment_id) {
    ...
    appointmentToComplete = {
      ...
      participantIds: appointment.participants.map((p) => p.client_id),
    };
  }
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (inlineReceiptNumber && inlineReceiptIssuedAt && amountStr) {
      ...
      const participantIds = new Set<string>([
        client_id,
        ...(appointmentToComplete?.participantIds ?? []),
        ...additionalParticipantIds,
      ]);
      if (appointmentToComplete || additionalParticipantIds.length > 0) {
        for (const participantClientId of participantIds) {
      ...
      if (scheduleNext && appointmentToComplete) {
        ...
        await tx.therapy_appointments.create({
          data: {
            ...
            household_id: householdId,
            client_id,
            family_id: primaryClient?.family_id ?? appointmentToComplete.family_id,
            job_id,
            program_id,
            visit_type,
            start_at: nextStart,
    ...
    if (error instanceof Error && error.message === "NEXT_APPOINTMENT_INVALID") {
      redirectPrivateClinicScoped(formData, "error", fallbackPath, "next_appt");
    }
  ...
  if (appointment_id) {
    ...
    const session = await getAuthSession();
    const userId = session?.user?.id;
    if (userId && appointmentToComplete) {
      const after = await prisma.therapy_appointments.findFirst({
        where: { id: appointmentToComplete.id, household_id: householdId },
...
export async function updateTherapyTreatment(formData: FormData) {
  ...
  if (amountRaw !== "" && amountStr == null) {
    redirectPrivateClinicScoped(formData, "error", fallbackPath, "amount");
  }
  if (!job_id || !occurred_date || !visit_type) {
    redirectPrivateClinicScoped(formData, "error", fallbackPath, "missing");
  }
  if (!(await assertJobForCurrentUserScope(householdId, userFm, job_id))) {
    redirectPrivateClinicScoped(formData, "error", fallbackPath, "job");
...
export async function createTherapyReceiptForSelectedTreatments(formData: FormData) {
  ...
  if (treatments.some((t) => t.amount == null)) {
    redirectPrivateClinicScoped(formData, "error", fallbackPath, "amount");
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const treatment of treatments) {
      const allocationAmount = treatment.amount!;
      await tx.therapy_receipt_allocations.upsert({
...
export async function linkTreatmentsToReceipt(formData: FormData) {
  ...
  if (treatments.some((t) => t.amount == null)) {
    redirect(`${BASE}/receipts/${receiptId}?error=treatmentamount`);
  }
  await prisma.$transaction(
    treatments.map((treatment) => {
      const allocationAmount = treatment.amount!;
      return prisma.therapy_receipt_allocations.upsert({
        where: { receipt_id_treatment_id: { receipt_id: receiptId, treatment_id: treatment.id } },
        create: {
          id: crypto.randomUUID(),
          household_id: householdId,
...
export async function saveTherapyConsultationTypeInline(
  id: string,
  name: string,
  name_he: string | null,
  householdIdForAdmin?: string,
): Promise<{ ok: true } | { ok: false }> {
  const session = await getAuthSession();
  let householdId: string;
  let isSuperAdminContext = false;

  if (session?.user?.isSuperAdmin && householdIdForAdmin) {
    await requireSuperAdmin();
    householdId = householdIdForAdmin;
    isSuperAdminContext = true;
  } else {
    await requireHouseholdMember();
    const currentHouseholdId = await getCurrentHouseholdId();
    if (!currentHouseholdId) return { ok: false };
    householdId = currentHouseholdId;
  }

  const trimmedName = name.trim();
  if (!id || !trimmedName) return { ok: false };

  const updated = await prisma.therapy_consultation_types.updateMany({
    where: { id, household_id: householdId },
    data: { name: trimmedName, name_he: name_he?.trim() || null },
  });
...
export async function createTherapyTravelEntry(formData: FormData) {
  ...
  const job_id = (formData.get("job_id") as string)?.trim() || "";
  const treatment_id = (formData.get("treatment_id") as string)?.trim() || "";
  const consultation_id = (formData.get("consultation_id") as string)?.trim() || "";
  const notes = (formData.get("notes") as string)?.trim() || null;
  const amountStr = parseMoney((formData.get("amount") as string) || null);
  const kmStr = parseOptionalKm((formData.get("km") as string) || null);
  ...
  if (amountStr == null) redirectPrivateClinicScoped(formData, "error", fallbackError, "amount");
  if (treatment_id && consultation_id) redirectPrivateClinicScoped(formData, "error", fallbackError, "link");
  if (!(await assertJobForCurrentUserScope(householdId, userFm, job_id))) {
    redirectPrivateClinicScoped(formData, "error", fallbackError, "job");
  }

  let treatmentId: string | null = null;
  let consultationIdOut: string | null = null;
  ...
  if (treatment_id) {
    ...
    if (t.job_id !== job_id) redirectPrivateClinicScoped(formData, "error", fallbackError, "job");
    treatmentId = t.id;
  }

  if (consultation_id) {
    const c = await assertConsultationForHousehold(householdId, consultation_id);
    if (!c) redirectPrivateClinicScoped(formData, "error", fallbackError, "consultation");
    if (c.job_id !== job_id) redirectPrivateClinicScoped(formData, "error", fallbackError, "job");
  ...
  await prisma.therapy_travel_entries.create({
    data: {
      ...
      job_id: job_id,
      treatment_id: treatmentId,
      consultation_id: consultationIdOut,
      occurred_at,
      notes,
      amount: amountStr,
      km: kmStr,
...
export async function updateTherapyTravelEntry(formData: FormData) {
  ...
  const row = await prisma.therapy_travel_entries.findFirst({
    where: { id, household_id: householdId },
    include: { treatment: { select: { job_id: true } }, consultation: { select: { job_id: true } } },
  });
  if (!row) redirect(`${BASE}/travel?error=notfound`);
  const priorJobId = row.job_id ?? row.treatment?.job_id ?? row.consultation?.job_id ?? null;
  ...
  const amountStr = parseMoney((formData.get("amount") as string) || null);
  if (amountStr == null) redirect(`${BASE}/travel?error=amount`);
  if (treatment_id && consultation_id) redirect(`${BASE}/travel?error=link`);
  ...
  if (treatment_id) {
    ...
    if (t.job_id !== job_id) redirect(`${BASE}/travel?error=job`);
    treatmentId = t.id;
  }

  if (consultation_id) {
    const c = await assertConsultationForHousehold(householdId, consultation_id);
    if (!c) redirect(`${BASE}/travel?error=consultation`);
    if (c.job_id !== job_id) redirect(`${BASE}/travel?error=job`);
    consultationIdOut = c.id;
  }
