...
export async function upsertClinicDigestEmailSubscription(formData: FormData) {
  ...
  const rawFreq = (formData.get("frequency") as string | null)?.trim() ?? "daily";
  const frequency = (FREQ.includes(rawFreq as RenewalEmailFrequency) ? rawFreq : "daily") as RenewalEmailFrequency;
  const send_hour = Math.min(23, Math.max(0, Number(formData.get("send_hour")) || 18));
  const days_ahead = Math.min(365, Math.max(1, Number(formData.get("days_ahead")) || 90));
  ...
  await prisma.clinic_digest_email_subscriptions.upsert({
  ...
  redirect(`${SETTINGS_PATH}?digest=saved`);
}

export async function disableClinicDigestEmailSubscription() {
  await requireHouseholdMember();
  const session = await getAuthSession();
  const userId = session?.user?.id;
  if (!userId || session.user.isSuperAdmin) return;

  await prisma.clinic_digest_email_subscriptions.updateMany({
    where: { user_id: userId },
    data: { is_active: false },
  });
  revalidatePath(SETTINGS_PATH);
  redirect(`${SETTINGS_PATH}?digest=disabled`);
}

export async function sendClinicDigestEmailTestNow() {
  await requireHouseholdMember();
  const session = await getAuthSession();
  const userId = session?.user?.id;
  const householdId = session?.user?.householdId;
  if (!userId || !householdId || session.user.isSuperAdmin) {
    redirect(`${SETTINGS_PATH}?digest=test-error`);
  }
  ...
  if (!sub) {
    redirect(`${SETTINGS_PATH}?digest=test-nosub`);
  }

  const result = await sendClinicDigestForSubscription(sub, new Date(), {
    updateLastSentAt: false,
    isTest: true,
  });
  if (!result.ok) {
    redirect(
      `${SETTINGS_PATH}?digest=test-fail&reason=${encodeURIComponent(result.reason.slice(0, 500))}`,
    );
  }
  redirect(`${SETTINGS_PATH}?digest=test-ok`);
}
