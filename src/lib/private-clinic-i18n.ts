...
export function privateClinicClients(lang: UiLanguage) {
  ...
  return {
    ...
    personalDetailsBtn: p("Personal details", "פרטים אישיים"),
    personalDetailsTitle: p("Personal details", "פרטים אישיים"),
    personalDetailsOnFile: p("Additional details on file", "פרטים נוספים שמורים"),
    personalDetailsDone: p("Done", "סיום"),
    overlayClose: p("Close", "סגירה"),
    formSectionIdentity: p("Identity", "זהות"),
    formSectionAssignment: p("Assignment", "שיבוץ"),
    formSectionDates: p("Dates", "תאריכים"),
    formSectionClinical: p("Clinical", "קליני"),
    formSectionBilling: p("Billing", "חיוב"),
    relManage: p("Manage", "ניהול"),
    ...
    holdPeriodsTitle: p("Hold periods", "תקופות השהיה"),
    holdPeriodsHelp: p(
      "Dated hospital stays or other holds. Days on hold are excluded from active-client counts. Returned is the day they came back to service — leave it empty while still on hold, then save.",
      "אשפוזים או השהיות אחרות עם תאריכים. ימי השהיה לא נספרים כלקוח פעיל. תאריך החזרה הוא היום שבו חזרו לשירות — השאירו ריק כל עוד מושהים, ואז שמרו.",
    ),
    holdStatusInService: p("In service", "בשירות"),
    holdOnHoldSince: (dateText: string) => p(`On hold since ${dateText}`, `מושהה מ־${dateText}`),
    holdColStart: p("Hold start", "תחילת השהיה"),
    holdColReturned: p("Returned", "חזרה לשירות"),
    holdColReason: p("Reason", "סיבה"),
    holdColNotes: p("Notes", "הערות"),
    holdStillOnHold: p("Still on hold", "עדיין מושהה"),
    holdReturnedHint: p("Leave empty while still on hold.", "השאירו ריק כל עוד מושהים."),
    holdReasonHospital: p("Hospital", "אשפוז"),
    holdReasonOther: p("Other", "אחר"),
    holdReasonNone: p("No reason", "ללא סיבה"),
    holdNotesPlaceholder: p("e.g. psychiatric home hospitalization", "למשל אשפוז בית פסיכיאטרי"),
    holdPlaceOnHold: p("Place on hold", "השהיה"),
    holdEmptyList: p("No hold periods yet.", "אין עדיין תקופות השהיה."),
    errHoldStart: p("Hold start date is required.", "נדרש תאריך תחילת השהיה."),
    errHoldDates: p("Return date cannot be before the hold start date.", "תאריך החזרה לא יכול להיות לפני תחילת ההשהיה."),
    errHoldOverlap: p("Hold periods cannot overlap.", "תקופות השהיה לא יכולות לחפוף."),
    errHoldOpen: p("This client already has an open hold period.", "ללקוח זה כבר יש תקופת השהיה פתוחה."),
    errHoldNotfound: p("Hold period not found.", "תקופת ההשהיה לא נמצאה."),
    errHoldReason: p("Hold reason is invalid.", "סיבת ההשהיה אינה תקינה."),
  };
}
...
export function privateClinicDashboard(lang: UiLanguage) {
  ...
  return {
    ...
    activeClientsByMonthHint: p(
      "A client counts in a month if they had at least one calendar day in care that was not a hospital or other hold day.",
      "לקוח נספר בחודש אם היה לו לפחות יום אחד בטיפול שאינו יום אשפוז או השהיה אחרת.",
    ),
    ...
    clientsInMonth: (monthLabel: string) =>
      p(
        `At least one non-hold care day in ${monthLabel}`,
        `לפחות יום טיפול אחד שאינו השהיה ב־${monthLabel}`,
      ),
