...
import {
  ...
  occurredAtToSplitDatetimeInitial,
...
test("parseTherapyOccurredAtDatetimeLocal treats T00:00 as date-only", () => {
  const d = parseTherapyOccurredAtDatetimeLocal("2026-07-08T00:00");
  assert.ok(d);
  assert.equal(d!.toISOString(), "2026-07-08T00:00:00.000Z");
});

test("parseTherapyOccurredAtDatetimeLocal interprets wall time as Israel", () => {
  const d = parseTherapyOccurredAtDatetimeLocal("2026-07-21T19:30");
  assert.ok(d);
  assert.equal(formatHouseholdDateUtcWithOptionalTime(d!, "YMD"), "2026-07-21 19:30");
  // Israel is UTC+3 in July — must not store as if the wall time were UTC
  assert.notEqual(d!.toISOString(), "2026-07-21T19:30:00.000Z");
});

test("occurredAtToSplitDatetimeInitial round-trips Israel wall time", () => {
  const d = parseTherapyOccurredAtDatetimeLocal("2026-07-21T19:30");
  assert.ok(d);
  assert.equal(occurredAtToSplitDatetimeInitial(d!), "2026-07-21T19:30");
});

test("occurredAtToSplitDatetimeInitial omits time for date-only rows", () => {
  const d = new Date("2026-07-08T00:00:00.000Z");
  assert.equal(occurredAtToSplitDatetimeInitial(d), "2026-07-08");
});
