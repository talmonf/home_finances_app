...
Run scripts **in numeric order** (001 → 002 → …), not in the order listed below. The table is **newest first** so the latest script is at the top.
...
Check off each script after you run it.
...
**Your checklist (mark when run; skip if your DB already has these).** Newest first — same order as the table above.
